...
- ✅ **S0. RLS / security hardening** — DONE (migration `0097`, see §2.5/§9 notes): RLS deny-all on all public tables + auto-RLS event trigger; closes the Supabase auto-API exposure (100 tables incl. `agent_api_keys`). Users are stored via **better-auth** (`user`/`session`/`account`/`verification` tables), not Supabase Auth.
- ✅ **N1. Hire-an-Analyst flow** — DONE. Seeded the 6 finance skills into the installable catalog (`packages/skills-catalog/catalog/optional/finance/*`, 15 total). `ui/src/lib/analystPersonas.ts` (6 personas) + `ui/src/components/HireAnalystDialog.tsx` (gallery → configure brain/adapter+model, inline "Connect API keys" via `ApiKeysWizard`, defaults adapter from connected `ai.*` key) → creates the agent via the existing `POST /agent-hires` with `desiredSkills:[skillKey]`. "Hire an Analyst" entry in the *Your Analysts* sidebar section. Follow-ups: hire a whole team at once; richer adapter↔key validation.
- ✅ **N2. De-Paperclip the onboarding/agent framing** — DONE (option A: relabel, keep mechanism). Onboarding wizard reframed to finance (research workspace + analysts, not company + CEO; finance default task/brief); first-agent name default "CEO"→"Research Lead"; **`AGENT_ROLE_LABELS.ceo` relabeled "CEO"→"Research Lead"** while keeping the `ceo` role value + its permissions (`canCreateAgents`/task-assign) intact. Deferred (option B): fully removing the CEO/board concept + reworking those permissions for a single-user model. Minor leftover: Company Export/Import view still shows a hardcoded "CEO" label (admin/org surface, not first-run).
- ⏳ **N3. Marketing site** — standalone **landing + docs + blog** (finance-rewritten, NOT Paperclip), on a static host (Vercel/Cloudflare Pages), app on an `app.` subdomain. **Docs** = a Learn/Reference docs site (per owner's reference images); **Blog** = a blog (per owner's images); the landing page's `Docs`/`Blog` links currently point to placeholders. Landing has **two CTAs: "Start free (hosted)"** → cloud sign-up, **"Self-host / Source"** → GitHub. Self-host vs hosted is a deploy-time `STOCKPILOT_MODE`, not an in-app toggle. **Owner will do the Vercel steps — pause and give exact instructions when we reach this.**
