...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Failed to update routine.");
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? `Failed to disable 1 timer routine: ${detail}`
            : `Failed to disable ${failures.length} of ${enabled.length} timer routines. First error: ${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Failed to disable all routines.");
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">Loading scheduler routines...</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : "Failed to load scheduler routines."}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Scheduler Routines</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Agents with a timer routine enabled across all of your companies.
        </p>
      </div>
        ...
        {anyEnabled && (
          <Button
            ...
            onClick={() => {
              ...
              if (!window.confirm(`Disable timer routines for all ${enabledCount} enabled ${noun}?`)) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message="No scheduler routines match the current criteria."
        />
      ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? "Disable Timer Routine" : "Enable Timer Routine"}
                          </Button>
                        </span>
                      </div>
                    );
