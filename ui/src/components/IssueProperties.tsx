  ...
  return (
    <>
      <span
        ...
        title={issue.title}
        aria-label={`Research task ${issueLabel}: ${issue.title}`}
      >
        <button
          type="button"
  ...
  const assigneeOptionsContent = supportsAssigneeOverrides ? (
    ...
    <div className="w-full space-y-2 p-2">
      <p className="text-xs text-muted-foreground">
        {assignee
          ? "This assignee's adapter does not expose editable research task overrides."
  ...
  const blockedByContent = (
    <>
      <input
        ...
        autoFocus={!inline}
        aria-label="Search research tasks to add as blockers"
      />
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          className={cn(
        ...
        {blockerOptionsLoading ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">Searching research tasks...</div>
        ) : blockerOptions.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">No matching research tasks.</div>
        ) : null}
      </div>
    </>
  );
