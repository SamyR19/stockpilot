  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder="Search research tasks..."
...
export function IssuesList({
  ...
  const createActionLabel = createIssueLabel ? `Create ${createIssueLabel}` : "Create Research Task";
  const createButtonLabel = createIssueLabel ? `New ${createIssueLabel}` : "New Research Task";
  ...
  return (
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <IssueColumnPicker
            ...
            title="Choose which research task columns stay visible"
            iconOnly
          />
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="No research tasks match the current filters or search."
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={`New research task in ${group.label}`}
                    aria-label={`New research task in ${group.label}`}
                          ...
                          <>
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                title="This research task needs a next step"
                              >
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? "Loading more research tasks..."
                  : remainingIssueRowCount > 0
                    ? `Rendering ${Math.min(renderedIssueRowLimit, filtered.length)} of ${filtered.length} research tasks`
                    : "Scroll to load more research tasks"}
              </p>
            </div>
          )}
        </>
