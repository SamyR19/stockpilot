...
const LIVENESS_COPY: Record<RunLivenessState, LivenessCopy> = {
  completed: {
    ...
    description: "Research task reached a terminal state.",
  },
  advanced: {
    label: "Advanced",
  ...
  blocked: {
    ...
    description: "Run or research task declared a blocker.",
  },
  failed: {
    label: "Failed",
...
export function IssueRunLedgerContent({
  ...
  return (
    <section className="space-y-3" aria-label="Research task run ledger">
      ...
      {feedItems.length === 0 ? (
        <div className="rounded-md border border-dashed border-border px-3 py-3 text-sm text-muted-foreground">
          {renderActivityEvent
            ? "Runs and activity will appear here once this research task has history."
            : "Historical runs without liveness metadata will appear here once linked to this research task."}
        </div>
      ) : (
