...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error("Select a workspace to detect the model");
      }
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error("Select a workspace to test adapter environment");
      }
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  Prompt template is replayed on every routine. Keep it compact and dynamic to avoid recurring token cost and cache churn.
                </div>
              </>
            )}
          </div>
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              ...
              hint="Agent-level default execution target. Project and research task settings can still override this."
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">Workspace default (Local)</option>
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel="sec"
              numberPrefix="Run routine every"
